...
namespace PhaseSpace
{
	...
	{			
		...
		protected OWLConnection [] servers = new OWLConnection[0];
		...
		{			
			OWLConnection [] conn = new OWLConnection[64];
			int ret = libowlsock.owl_scan(null, 8999, "unity owl wrapper", timeout_usec, conn, conn.Length);
			OWLConnection [] conn2 = new OWLConnection[0];
			if(ret > 0)
			{
				conn2 = new OWLConnection[ret];
				Array.Copy(conn, conn2, ret);
			}
			return conn2;			
		}
		...
		static protected void ConvertServers(OWLConnection [] owlservers, out Server [] servers)
		{
			...
			{					
				servers[i] = new Server(new System.String(owlservers[i].address),
				                        new System.String(owlservers[i].info));
			}						
		}
